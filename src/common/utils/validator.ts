...
export const validator = (values: ValuesType, setErrorHandler: (error: FormErrorType) => void) => {
    const errors: FormErrorType = {}

    if (values.name === "") {
        errors.name = EMPTY_FIELD_ERROR;
    } else if (values.name && values.name.charAt(0) !== values.name.charAt(0).toUpperCase()) {
        errors.name = "Имя должно начинаться с заглавной буквы";
    }

    if (values.surname === "") {
        errors.surname = EMPTY_FIELD_ERROR;
    } else if (values.surname && values.surname.charAt(0) !== values.surname.charAt(0).toUpperCase()) {
